...
class RuleConverter:
    ...
    def convert(self, input_directory: str, output_directory: str) -> None:
        for root, _, files in os.walk(input_directory):
            for filename in [name for name in files if name.endswith(".xml")]:
                ...
                output_name = identifier(rel_path.removesuffix(".xml"))
    ...
    def __collect_rules_from_file(self, rule_file: pathlib.Path) -> list[dict[str, Any]]:
        ...
        try:
            ...
            print(
                f"[ERROR] Could not parse rule file {rule_file}: {exc}",
                file=sys.stderr,
            )
    ...
    def __generate_pytest_code(self, rules: list[dict[str, Any]]) -> str:
        ...
        for rule in rules:
            ...
            groups = rule["groups"]
            test_name = identifier(f"rule_{rule_id}")
            ...
            lines.append(f"    assert response.rule_level == {int(level) if level is not None else None}")
            lines.append(
