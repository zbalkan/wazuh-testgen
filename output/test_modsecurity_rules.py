...
class TestModsecurityRules(unittest.TestCase):
    ...
    def test_modsecurity_audit_log_messages_grouped_1(self) -> None:
        log = r'''[Mon Feb 09 21:17:06.798110 2015] [:error] [pid 8608] [client 172.16.10.57] ModSecurity: Audit log: Failed writing (requested 83 bytes, written 24): No space left on device [hostname "172.16.10.91"] [uri "/403.php"] [unique_id "VNk-8n8AAQEAACGg7LEAAAAE"]'''
        response = send_log(log)
    ...
    def test_modsecurity_audit_log_messages_grouped_2(self) -> None:
        log = r'''[Wed Feb 11 19:46:12.759594 2015] [:error] [pid 1130] [client 172.16.10.91] ModSecurity: Audit log: Failed to lock global mutex: Identifier removed [hostname "172.16.10.91"] [uri "/wordpress/wp-cron.php"] [unique_id "VNvLw38AAQEAAARqTXsAAAAD"]'''
        response = send_log(log)
