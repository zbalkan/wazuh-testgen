...
class TestSshdRules(unittest.TestCase):
    ...
    def test_sshd_configuration_error_authorizedkeyscommand(self) -> None:
        log = r'''
Feb  9 11:44:56 someserver sshd[1234]: error: Could not stat AuthorizedKeysCommand "/usr/local/sbin/ssh-ldap-authorized_keys": No such file or directory
'''
    ...
    def test_ssh_connection_reset_by_peer(self) -> None:
        log = r'''
Feb 10 23:21:05 someserver sshd[1234]: Read error from remote host 192.168.1.1: Connection reset by peer
'''
        response = send_log(log)
    ...
    def test_ssh_connection_refused(self) -> None:
        log = r'''
Feb 11 06:41:50 someserver sshd[1234]: debug1: channel 5: connection failed: Connection refused
'''
        response = send_log(log)
    ...
    def test_ssh_connection_timed_out(self) -> None:
        log = r'''
Feb 12 17:45:09 someserver sshd[1234]: debug1: channel 3: connection failed: Connection timed out
'''
        response = send_log(log)
    ...
    def test_ssh_no_route_to_host(self) -> None:
        log = r'''
Jan 30 18:55:24 someserver sshd[1234]: debug1: channel 1: connection failed: No route to host
'''
        response = send_log(log)
    ...
    def test_ssh_port_forwarding_issue(self) -> None:
        log = r'''
Feb 13 22:54:51 someserver sshd[1234]: debug1: server_input_channel_open: failure direct-tcpip
'''
        response = send_log(log)
    ...
    def test_ssh_transport_endpoint_is_not_connected(self) -> None:
        log = r'''
Feb  6 12:28:17 someserver sshd[1234]: debug1: getpeername failed: Transport endpoint is not connected
'''
        response = send_log(log)
    ...
    def test_ssh_get_remote_port_failed(self) -> None:
        log = r'''
Feb  6 12:28:17 someserver sshd[1234]: debug1: get_remote_port failed
'''
        response = send_log(log)
    ...
    def test_ssh_bad_client_public_dh_value_1(self) -> None:
        log = r'''
Feb  4 23:05:57 someserver sshd[1234]: Disconnecting: bad client public DH value [preauth]
'''
        response = send_log(log)
    ...
    def test_ssh_bad_client_public_dh_value_2(self) -> None:
        log = r'''
Feb  4 23:05:57 someserver sshd[1234]: Disconnecting: bad client public DH value
'''
        response = send_log(log)
    ...
    def test_ssh_corrupted_mac_on_input_1(self) -> None:
        log = r'''
Feb 14 14:34:15 someserver sshd[1234]: Corrupted MAC on input. [preauth]
'''
        response = send_log(log)
...
'''
    ...
    def test_ssh_bad_packet_length_1(self) -> None:
        log = r'''
Mar  4 13:34:59 someserver sshd[5396]: Bad packet length 4081586742. [preauth]
'''
        response = send_log(log)
    ...
    def test_ssh_bad_packet_length_2(self) -> None:
        log = r'''
Mar  4 13:34:59 someserver sshd[5396]: Bad packet length 4081586742.
'''
        response = send_log(log)
    ...
    def test_ssh_unable_to_negotiate(self) -> None:
        log = r'''
Mar  3 10:56:18 junction sshd[32065]: fatal: Unable to negotiate with 202.191.177.33 port 3579: no matching cipher found. Their offer: 3des-cbc,aes128-cbc,aes192-cbc,aes256-cbc [preauth]
'''
        response = send_log(log)
    ...
    def test_ssh_no_matching_key_exchange_1(self) -> None:
        log = r'''
Sep 16 05:46:56 junction sshd[1961]: fatal: Unable to negotiate with 108.229.36.174: no matching key exchange method found. Their offer: diffie-hellman-group1-sha1,diffie-hellman-group-exchange-sha1 [preauth]
'''
        response = send_log(log)
    ...
    def test_ssh_no_matching_key_exchange_2(self) -> None:
        log = r'''
Apr 18 21:27:08 web2 sshd[23484]: fatal: Unable to negotiate a key exchange method [preauth]
'''
        response = send_log(log)
...
'''
    ...
    def test_invalid_user_5(self) -> None:
        log = r'''
2020-03-23 06:47:42.801612-0700  localhost sshd[3186]: error: PAM: unknown user for illegal user badguy from 192.168.33.1
'''
        response = send_log(log)
    ...
    def test_invalid_user_6(self) -> None:
        log = r'''
2020-03-25 08:01:34.584936-0700  localhost sshd[1551]: Failed keyboard-interactive/pam for invalid user user from 172.18.1.1 port 32982 ssh2
'''
        response = send_log(log)
...
'''
    ...
    def test_invalid_user_8(self) -> None:
        log = r'''
2020-03-23 08:14:02.777660-0700  localhost sshd[8981]: error: PAM: authentication error for illegal user badguy from 192.168.33.1
'''
        response = send_log(log)
...
'''
    ...
    def test_failed_to_create_session(self) -> None:
        log = r'''
May  4 17:48:43 collectd sshd[15044]: pam_systemd(sshd:session): Failed to create session: Access denied
'''
        response = send_log(log)
    ...
    def test_bad_authorized_keys(self) -> None:
        log = r'''
May  4 18:30:04 collectd sshd[15191]: Authentication refused: bad ownership or modes for file /home/ansible/.ssh/authorized_keys
'''
        response = send_log(log)
    ...
    def test_subsystem_failed(self) -> None:
        log = r'''
May  5 05:00:38 junction sshd[28395]: subsystem request for netconf by user checker failed, subsystem not found
'''
        response = send_log(log)
    ...
    def test_login_failed(self) -> None:
        log = r'''
Aug 18 07:30:25 192.168.1.5 sshd[20247]: [ID 800047 auth.notice] Failed none for root from 192.168.1.1 port 36942 ssh2
'''
        response = send_log(log)
    ...
    def test_bad_dns_1(self) -> None:
        log = r'''
Oct 20 12:33:07 ar-agent sshd[3433]: Address 192.168.18.54 maps to nmap.18.168.192.in-addr.arpa, but this does not map back to the address - POSSIBLE BREAK-IN ATTEMPT!
'''
        response = send_log(log)
    ...
    def test_bad_dns_2(self) -> None:
        log = r'''
2020-03-25 09:01:30.852002-0700  localhost sshd[11885]: Address 192.168.33.1 maps to hostname, but this does not map back to the address - POSSIBLE BREAK-IN ATTEMPT!
'''
        response = send_log(log)
...
'''
    ...
    def test_max_auth_attempts_2(self) -> None:
        log = r'''
2020-03-23 08:14:32.766049-0700  localhost sshd[8981]: error: maximum authentication attempts exceeded for invalid user badguy from 192.168.33.1 port 55146 ssh2 [preauth]
'''
        response = send_log(log)
    ...
    def test_max_auth_attempts_3(self) -> None:
        log = r'''
2020-03-23 09:58:27.102292-0700  localhost sshd[18093]: error: maximum authentication attempts exceeded for user from 192.168.33.1 port 55764 ssh2 [preauth]
'''
        response = send_log(log)
    ...
    def test_sshd_authentication_error_1(self) -> None:
        log = r'''
2020-03-23 09:55:42.391078-0700  localhost sshd[17329]: error: PAM: authentication error for user from 192.168.33.1
'''
        response = send_log(log)
    ...
    def test_sshd_authentication_error_2(self) -> None:
        log = r'''
2020-03-24 08:38:42.344447-0700  localhost sshd[2519]: Failed password for user from 172.18.1.100 port 43042 ssh2
'''
        response = send_log(log)
    ...
    def test_sshd_connection_close(self) -> None:
        log = r'''
2020-03-24 06:07:15.245255-0700  localhost sshd[195]: Connection closed by 10.0.2.2 port 55462 [preauth]
'''
        response = send_log(log)
...
'''
    ...
    def test_sshd_insecure_connection_attempt(self) -> None:
        log = r'''
2020-03-24 10:32:31.672920-0700  localhost sshd[5374]: Did not receive identification string from 172.18.1.1 port 45824
'''
        response = send_log(log)
...
'''
    ...
    def test_sshd_denied_user_1(self) -> None:
        log = r'''
2020-03-25 07:46:15.205351-0700  localhost sshd[6738]: User root from 192.168.33.1 not allowed because not listed in AllowUsers
'''
        response = send_log(log)
    ...
    def test_sshd_denied_user_2(self) -> None:
        log = r'''
2020-03-31 13:15:57.368975-0700  localhost sshd[2440]: User root from 172.18.1.100 not allowed because listed in DenyUsers
'''
        response = send_log(log)
...
'''
    ...
    def test_sshd_reverse_lookup_error(self) -> None:
        log = r'''
2020-03-25 09:18:41.510217-0700  localhost sshd[2549]: reverse mapping checking getaddrinfo for hostname [172.18.1.1] failed - POSSIBLE BREAK.
'''
        response = send_log(log)
    ...
    def test_sshd_possible_attack(self) -> None:
        log = r'''
2020-03-25 06:37:50.176931-0700  localhost sshd[852]: Bad protocol version identification 'ls' from 172.18.1.1 port 59920
'''
        response = send_log(log)
...
'''
    ...
    def test_sshd_brute_force_rule_2_1(self) -> None:
        log = r'''
May 29 11:31:00 vagrant sshd[30016]: Invalid user user from 212.64.151.233
'''
        response = send_log(log)
