...
class TestF5BigIpRules(unittest.TestCase):
    ...
    def test_high_demand_traffic(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010251:0: Virtual componentName exceeded configured rate limit.'''
    ...
    def test_syn_flood_attack(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010343:0: Syncookie SW mode activated, server = 1.1.1.1:4000'''
        response = send_log(log)
    ...
    def test_stopped_throttling_traffic(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 011e0001:0: Limiting componentName from 40 to 40 packets/sec for traffic-group componentName'''
        response = send_log(log)
    ...
    def test_syn_cookie_threshold_is_reached(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010038:0: Syncookie counter 40 exceeded vip threshold %u for virtual = 1.1.1.1:4000'''
        response = send_log(log)
    ...
    def test_detected_a_syncookie_dos_attack(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010240:0: Syncookie HW mode activated, server = 1.1.1.1:4000, HSB modId = 40'''
        response = send_log(log)
    ...
    def test_ongoing_ddos_attack(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010329:0: BDoS: (TMM) Signature componentName: threshold_mode=componentName detection=%u mitigation_curr=%llu'''
        response = send_log(log)
    ...
    def test_created_updated_afm_bdos_dynamic_signature_by_the_afm_bdosd_daemon_during_an_attack(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010302:0: BDoS: (TMM) componentName signature (componentName) for context componentName at idx %u (detection=%u mitigation=%u state=componentName transient=componentName retired=componentName).'''
        response = send_log(log)
    ...
    def test_number_of_allowed_new_connections_per_second_for_pool_member_has_been_exceeded(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010250:0: Pool member %A:%u exceeded configured rate limit.'''
        response = send_log(log)
    ...
    def test_syncookie_dos_attack_has_stopped(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010241:0: Syncookie HW mode exited, server = 1.1.1.1:4000, HSB modId = 40 from componentName.'''
        response = send_log(log)
    ...
    def test_syncookie_counter_exceeded_vip_threshold(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010056:0: Syncookie counter 40 exceeded vip threshold %u for virtual = componentName'''
        response = send_log(log)
    ...
    def test_syn_cookie_state_exited(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010344:0: Syncookie SW mode exited, server = 1.1.1.1:4000'''
        response = send_log(log)
    ...
    def test_sslv2_is_no_longer_supported_and_has_been_removed(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01071bee:0: SSLv2 is no longer supported and has been removed. The 'sslv2' keyword in the cipher string has been ignored.'''
        response = send_log(log)
    ...
    def test_user_is_prevented_from_doing_things_they_are_not_authorized_to_do(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01070822:0: "Access Denied: componentName"'''
        response = send_log(log)
    ...
    def test_mcpd_has_detected_that_sync_traffic_is_being_sent_over_a_vlan_that_is_not_the_correct_one(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01071bd1:0: Inbound CMI connection from IP (componentName) denied because it came from VLAN (componentName), not from expected VLAN (componentName).'''
        response = send_log(log)
    ...
    def test_too_many_sip_media_sessions_have_been_established_for_the_current_configuration(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01860017:0: MR_SIP: Too many media sessions 40 / 40. Error Code 40'''
        response = send_log(log)
    ...
    def test_critical_error_for_tmm_it_restarts_attempts_to_reconnect_will_be_made_after_that(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01010020:0: MCP Connection componentName, exiting'''
        response = send_log(log)
    ...
    def test_errors_could_be_caused_by_a_broken_feature_or_critical_system_errors(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01071d0b:0: adm: componentName'''
        response = send_log(log)
    ...
    def test_the_hal_daemon_might_not_be_able_to_correctly_identify_the_platform_or_publish_the_hardware_abstraction_configuration_at_startup_or_has_encountered_a_critical_failure_during_normal_operation_1(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 012a0002:0: "LIBHAL reporting critical conditions"'''
        response = send_log(log)
    ...
    def test_the_hal_daemon_might_not_be_able_to_correctly_identify_the_platform_or_publish_the_hardware_abstraction_configuration_at_startup_or_has_encountered_a_critical_failure_during_normal_operation_2(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 012a0003:0: LIBHAL reporting error conditions'''
        response = send_log(log)
    ...
    def test_hardware_sensor_critical_alarm(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 012a0013:0: Blade 40 hardware sensor critical alarm: componentName'''
        response = send_log(log)
    ...
    def test_aom_has_indicated_that_a_temperature_sensor_has_crossed_a_critical_level_threshold(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 012a0031:0: componentName'''
        response = send_log(log)
    ...
    def test_aom_has_indicated_that_a_fan_sensor_has_crossed_a_critical_threshold(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 012a0037:0: componentName'''
        response = send_log(log)
    ...
    def test_aom_has_indicated_that_a_power_sensor_has_crossed_a_critical_threshold(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 012a0043:0: componentName'''
        response = send_log(log)
    ...
    def test_critical_error_that_prevents_the_broadcom_switch_from_operating_at_the_proper_configuration_required_by_big_ip(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 012c0011:0: BCM56XXD SDK error'''
        response = send_log(log)
    ...
    def test_critical_errors_in_communication_between_tmm_threads_specifically_by_mpi_proxy(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01340003:0: Cluster error: componentName'''
        response = send_log(log)
    ...
    def test_serious_issue_preventing_the_guest_from_starting_or_shutting_down(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01510003:0: componentName'''
        response = send_log(log)
    ...
    def test_critical_the_big_ip_system_is_not_allowed_not_to_go_active_1(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01550004:0: Critical:'''
        response = send_log(log)
    ...
    def test_critical_the_big_ip_system_is_not_allowed_not_to_go_active_2(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01550005:0: Critical:'''
        response = send_log(log)
    ...
    def test_critical_the_big_ip_system_is_not_allowed_not_to_go_active_3(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01550006:0: Critical:'''
        response = send_log(log)
    ...
    def test_critical_the_errdefsd_daemon_is_out_of_memory(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01940007:0: "Failed to allocate the errdefs tmconf handle!"'''
        response = send_log(log)
    ...
    def test_critical_the_file_platform_isnt_found_and_licensing_logic_cannot_determine_the_platform_type(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01a70028:0: The platform was not found in componentName.'''
        response = send_log(log)
    ...
    def test_bot_defense_1(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01071d94:0: Bot Defense Profile (componentName) Micro Service (componentName): Missing required field (componentName).'''
        response = send_log(log)
    ...
    def test_bot_defense_2(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 01071d9e:0: Bot defense anomaly componentName not found.'''
        response = send_log(log)
    ...
    def test_tcp_dump(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 013e0002:0: Tcpdump stopping on %la:%u from %la:%u'''
        response = send_log(log)
    ...
    def test_tcp_dump_remote_session(self) -> None:
        log = r'''May  5 04:26:19 hostname type process[20175]: 013e0005:0: Tcpdump starting remote to %A from %A'''
        response = send_log(log)
    ...
    def test_asm_error(self) -> None:
        log = r'''May  4 13:42:12 some.host.name crit server_handler.pl[24895]: 01310027:2: ASM subsystem error (asm_config_server.pl,(eval)): Couldn't pass call to async process - ignoring'''
        response = send_log(log)
    ...
    def test_f5_bigip_info_message_detected(self) -> None:
        log = r'''May  5 04:26:19 hostname info process[20175]: 01011111:0: MCP Connection %s, exiting'''
        response = send_log(log)
    ...
    def test_f5_bigip_notice_message_detected(self) -> None:
        log = r'''May  5 04:26:19 hostname notice process[20175]: 01011111:0: MCP Connection %s, exiting'''
        response = send_log(log)
    ...
    def test_f5_bigip_warning_message_detected(self) -> None:
        log = r'''May  5 04:26:19 hostname warning process[20175]: 01011111:0: MCP Connection %s, exiting'''
        response = send_log(log)
    ...
    def test_f5_bigip_alert_message_detected(self) -> None:
        log = r'''May  5 04:26:19 hostname alert process[20175]: 01011111:0: MCP Connection %s, exiting'''
        response = send_log(log)
    ...
    def test_f5_bigip_critical_message_detected(self) -> None:
        log = r'''May  5 04:26:19 hostname crit process[20175]: 01011111:0: MCP Connection %s, exiting'''
        response = send_log(log)
